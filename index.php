...
if(isset($_POST['add_to_cart'])){
   ...
   if(mysqli_num_rows($select_cart) > 0){
      ...
      '$product_image', '$product_quantity')") or die('query failed');
...
if(isset($_POST['update_cart'])){
   ...
   mysqli_query($conn, "UPDATE `cart` SET quantity = '$update_quantity' 
...
if(isset($_GET['remove'])){
   ...
   mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$remove_id'") or 
   die('query failed');
...
<div class="shopping-cart">
   ...
   <table>
      ...
      ?>
         <tr>
            <td><img src="images/<?php echo $fetch_cart['image']; ?>" 
            height="100" alt=""></td>
            ...
            $fetch_cart['quantity']); ?>/-</td>
            <td><a href="index.php?remove=<?php echo $fetch_cart['id']; ?>" 
            class="delete-btn" onclick="return confirm('remove item from cart?');"
            >remove</a></td>
         </tr>
      <?php
         $grand_total += $sub_total;
            ...
            text-transform:capitalize;" colspan="6">no item added</td></tr>';
         }
      ?>
      <tr class="table-bottom">
         ...
         class="delete-btn <?php echo ($grand_total > 1)?'':
         'disabled'; ?>">delete all</a></td>
      </tr>
   </tbody>
   </table>

   <div class="cart-btn">  
      <a href="#" class="btn <?php echo ($grand_total > 1)?'':
